...
- intro_method: 統計（累計契約高・オーナー数）＋新チーム発足という宣言
- heading_structure:
  1. 組織の内側からビジネスを変革する
  2. 「広さ×深さ×速度」に挑む
...
- has_examples: あり（Salesforce活用事例・稼働率最適化・ルームプラン策定など具体的業務事例）
- has_visuals: あり（登壇者集合写真・スライド画像）
...
- voice: STORYTELLER
- formality: neutral
- first_person: 私／僕（登壇者による発言引用のため混在）
- reader_distance: 普通
- emotion_temp: warm
- has_humor: なし
- signature_phrases:
...
## D. 読者・ターゲット分析
- target_reader: スタートアップや成長企業でデータ・IT・AI領域のキャリアを描きたいエンジニア・BizOps人材。特に「ただのシステム管理者に留まりたくない」と感じている人。
- reader_pain: テクノロジー職でありながらビジネスインパクトを実感しにくく、守りの仕事に閉じてしまっているもどかしさ。
- reader_transformation: 「攻めのIT・データ組織」というキャリア像を具体的にイメージでき、転職候補としてNOT A HOTELが視野に入る。

---

## E. コンテンツ戦略分析
- title_hook: 逆説（"NOT A HOTEL"というブランド名の逆説性）＋新設・変化の予告
- unique_angle: 採用広報でありながら、登壇者3名の生の発言を軸に「なぜこのチームが面白いのか」を読者自身に発見させる構成。一方的なPRではなく、イベントレポートという中立的フォーマットを使っているのが差別化点。
- claim_strength: moderate
- cta_type: 採用促進（カジュアル面談への誘導）

---

## F. AI模倣用サマリー

- **一言で言うと：** 「攻めのIT・データ組織を作りたい」エンジニア・BizOps人材に向けて、新チームの思想と具体的業務をイベントレポート形式で伝える採用広報記事。
- **模倣すべき最大の強み：** 複数登壇者の発言を引用しながら、「広さ×深さ×速度」という独自フレームを軸に情報を整理することで、読者が組織の魅力を体系的に理解できる構成。
- **このパターンが活きるネタ：** 新チーム・新制度の紹介、採用広報、社内プロジェクトのドキュメント、イベントレポート形式のコンテンツ。
- **避けるべき使い方：** 一人の筆者の強い主張を前面に出したいOPINION系ネタや、個人の体験談を軸にしたSTORY系ネタには合わない。
- **文体模倣のポイント：** 登壇者・担当者のコメントを「：」で区切って直接引用し、三者三様の視点を交差させることでリアリティを演出する。見出しに「キャッチコピー的なフレーズ（例：10倍ではなく、100倍の変化を目指す野心）」を置いて読者の期待感を高める。
...
【資料公開】基調講演「クラウド時代のあるべき情シス、「モダンな情シス」の全貌」を公開しました。 - kajinari「Digital Leaders Summit Vol.2 2024 秋」の基調講演の機会を頂き登壇させていただきましたの
